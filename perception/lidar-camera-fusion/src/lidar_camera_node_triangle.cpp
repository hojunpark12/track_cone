...
// input topics 
std::string imgTopic = "/usb_cam/image2";
std::string imgTopic_blue = "/usb_cam/image1";
...
///////// Range 이미지에서 포인트 클라우드로 변환
  ...
  for (int j = 0; j < msg_len; j++)
  {
    ...
    int max_y_value = std::max( msg->poses[j].orientation.z,  msg->poses[j].orientation.x);

    bbox_ymax[j] = fmod(max_y_value,1000.0);
    bbox_ymin[j] = fmod(msg->poses[j].orientation.y,1000.0);

    double z_div_1000 = msg->poses[j].orientation.z / 1000.0;
    double x_div_1000 = msg->poses[j].orientation.x / 1000.0;

    if (z_div_1000 < 640) 
    {
        bbox_xmax[j] = z_div_1000;
        bbox_xmin[j] = x_div_1000;
    } 
    else 
    {
        bbox_xmax[j] = z_div_1000 - 640;
        bbox_xmin[j] = std::max(x_div_1000 - 640, 0.0);
    }

    if(msg->poses[j].orientation.z/1000 < 640)
    {
      ...
      for (int i = 0; i < size_inter_Lidar; i++)
      {
        ...
        if(((int)px_data - bbox_xmin[j])== 0 || (bbox_xmax[j]- (int)px_data) == 0 )
          continue;

        int left_slope = -(bbox_ymin[j]- bbox_ymax[j])/(msg->poses[j].orientation.y/ 1000 - bbox_xmin[j]) ;
        int right_slope = -(bbox_ymax[j] - bbox_ymin[j]) / (bbox_xmax[j]- msg->poses[j].orientation.y/ 1000);

        int left_p_slope = -((int)py_data - bbox_ymax[j])/((int)px_data - bbox_xmin[j]);
        int right_p_slope = -(bbox_ymax[j] - (int)py_data) / (bbox_xmax[j]- (int)px_data);

        //if(left_slope < left_p_slope || right_slope > right_p_slope )
          //continue;
        ...
        id_color->header.frame_id = (msg->poses[j].orientation.w == 0) ? "y" : "b";
  ...
  if( msg->poses[j].orientation.z/1000 > 640 )
  {
    ...
    for (int i = 0; i < size_inter_Lidar; i++)
    {
      ...
      int left_slope = -(bbox_ymin[j]- bbox_ymax[j])/(msg->poses[j].orientation.y/ 1000- 640 - bbox_xmin[j]) ;
      int right_slope = -(bbox_ymax[j] - bbox_ymin[j]) / (bbox_xmax[j] - (msg->poses[j].orientation.y/ 1000 - 640) );
      int left_p_slope = -((int)py_data - bbox_ymax[j])/((int)px_data - bbox_xmin[j]);
      int right_p_slope = -(bbox_ymax[j] - (int)py_data) / (bbox_xmax[j]- (int)px_data);

      //std::cout << ", " <<bbox_ymin[j]<< ", "  <<bbox_ymax[j]<< ", " <<bbox_xmin[j]<< ", " <<bbox_xmax[j]<< ", "  << msg->poses[j].orientation.y/ 1000 - 640 << std::endl;
