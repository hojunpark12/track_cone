...
def euler_rotation_matrix(alpha,beta,gamma): # x,y,z 축의 순서로 계산됨 
    """
...
def inverse_rotation(p2,alpha,beta,gamma):   # z yx 순서대로 계산함
    """
    ...
    inv_rot_matrix = np.array([[r00, r01, r02],
                               [r10, r11, r12],
                               [r20, r21, r22]]) 
    print(np.array2string(inv_rot_matrix, separator=', '))                   
...
def main():
    ...
    # Rotation angles 계산순서 x-> y-> z 시계방향이 + 반시계 - 
    # 카메라가 라이더로부터 어떻게 회전해있나?? x 반시계 90 z축 반시계90
    alpha = (-20/180.0)*np.pi # 라이더 좌표계 y -> 카메라 좌표계 x 
    beta = (-30/180.0)*np.pi # 라이더 좌표계 z -> 카메라 좌표계 y
    gamma = (0/180.0)*np.pi # 라이더 좌표계 x -> 카메라 좌표계 z
    #alpha = -1.9557
    #beta = 0.1197
    #gamma = -1.2955
